...
int main(int argc, char **argv) {
    ...
    if (!write_value(good, "abc") || !write_value(empty, "") ||
        !write_value(bad, "wrong"))
        return 1;
    text = read_all(argv[1], &length);
    if (!text || !cbs_lex(argv[1], text, length, &tokens))
        return 1;
    doc = cbs_parse(argv[1], text, length, &tokens);
    if (!doc || !cbs_validate(doc, argv[1], text) ||
        !cbs_sources_from_document(doc, &set))
        ok = 0;
    if (ok &&
        (set.count != 2 || set.items[0].url_count != 2 ||
         strcmp(set.items[0].urls[1], "https://mirror.invalid/upstream") ||
         cbs_sources_apply_execution_context(&set, &context)))
        ok = 0;
    if (ok &&
        !cbs_source_verify(&set.items[0], good, argv[1], text,
                           set.items[0].urls ? doc->location : doc->location))
        ok = 0;
    ...
    if (ok && test_capture_begin(&capture, base)) {
        ...
        if (!strstr(diagnostic, "CPDL-E5001") ||
            ...
            !strstr(diagnostic, "computed 8810"))
            ok = 0;
    } else
        ok = 0;
    if (ok && (!cbs_source_verify(&set.items[1], empty, argv[1], text,
                                  doc->location) ||
               !cbs_sources_apply_execution_context(&set, &context)))
        ok = 0;
    if (ok) {
        resolved = cbs_resolve_value("${source.upstream}|${source.empty}",
    ...
    puts("named source tests: PASS (mirrors, verification, gated bindings)");
    return 0;
}

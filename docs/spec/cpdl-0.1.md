...
```ebnf
mkdir-operation   = "mkdir", path-value, [ ( "parents" | "leaf" ) ],
                     [ "chmod", mode ] ;
...
`mkdir` creates missing path components, preserving the historical CPDL
behavior. Existing directories are accepted; an existing non-directory fails.
Its default final mode is `0755`, filtered only by explicit CBS policy recorded
in build metadata. The optional `parents` keyword is an explicit spelling of
the default recursive behavior. The optional `leaf` keyword opts into creating
only the final component; missing parents then fail rather than being created.

/* SHA-256 verification, cache binding, and source preparation. */
#include "cbs.h"
...
static void transform(Sha256 *sha, const unsigned char block[64]) {
    ...
    for (; i < 64; ++i) {
        uint32_t x = rotate_right(w[i - 15], 7) ^ rotate_right(w[i - 15], 18) ^
                     (w[i - 15] >> 3);
        uint32_t y = rotate_right(w[i - 2], 17) ^ rotate_right(w[i - 2], 19) ^
                     (w[i - 2] >> 10);
...
static void finish(Sha256 *sha, unsigned char digest[32]) {
    ...
    while (sha->used < 56)
        sha->block[sha->used++] = 0;
    for (i = 0; i < 8; ++i)
        sha->block[56 + i] = (unsigned char)(sha->bits >> (56 - 8 * i));
    transform(sha, sha->block);
    for (i = 0; i < 32; ++i)
...
int cbs_digest_text(const char *text, size_t length, char output[65]) {
    ...
    if (text == NULL || output == NULL)
        return 0;
    initialize(&sha);
    update(&sha, (const unsigned char *)text, length);
    finish(&sha, digest);
    for (i = 0; i < 32; ++i) {
        output[i * 2] = hex[digest[i] >> 4];
        output[i * 2 + 1] = hex[digest[i] & 15];
...
int cbs_sources_from_document(const CbsNode *document, CbsSourceSet *set) {
    ...
    for (i = 0; i < package->child_count; ++i)
        if (package->children[i]->kind == CBS_NODE_SOURCES)
            block = package->children[i];
    if (block == NULL)
        return 1;
    set->items = cbs_allocate(block->child_count * sizeof(*set->items));
    set->count = block->child_count;
    for (i = 0; i < set->count; ++i) {
        const CbsNode *node = block->children[i];
        CbsSource *source = &set->items[i];
        source->kind = node->name;
        source->name = node->value;
        source->url_count = node->child_count - 1;
        source->urls = cbs_allocate(source->url_count * sizeof(*source->urls));
        for (j = 0; j < source->url_count; ++j)
            source->urls[j] = node->children[j]->value;
        source->sha256 = node->children[node->child_count - 1]->value;
    }
    return 1;
}
...
void cbs_source_set_destroy(CbsSourceSet *set) {
    size_t i;
    for (i = 0; i < set->count; ++i) {
        free(set->items[i].urls);
        free(set->items[i].verified_path);
    }
    free(set->bindings);
    free(set->items);
    memset(set, 0, sizeof(*set));
...
int cbs_source_verify(CbsSource *source, const char *path,
    ...
    char actual[65], message[512];
    free(source->verified_path);
    source->verified_path = NULL;
    if (!cbs_digest_file(path, actual)) {
        snprintf(message, sizeof(message),
                 "source `%s` could not be hashed; errno=%d", source->name,
                 errno);
        cbs_diagnostic(recipe_path, recipe_source, location, "error",
                       "CPDL-E5001", CBS_DIAG_SOURCE, message);
        return 0;
    }
    if (strcmp(actual, source->sha256) != 0) {
        snprintf(message, sizeof(message),
                 "source `%s` checksum mismatch; expected %s; computed %s",
                 source->name, source->sha256, actual);
        cbs_diagnostic(recipe_path, recipe_source, location, "error",
                       "CPDL-E5001", CBS_DIAG_SOURCE, message);
        return 0;
    }
    source->verified_path = cbs_duplicate(path);
    return 1;
}
...
int cbs_sources_apply_execution_context(CbsSourceSet *set,
    ...
    size_t i;
    for (i = 0; i < set->count; ++i)
        if (set->items[i].verified_path == NULL)
            return 0;
    free(set->bindings);
    set->bindings = cbs_allocate(set->count * sizeof(*set->bindings));
    for (i = 0; i < set->count; ++i) {
        set->bindings[i].name = set->items[i].name;
        set->bindings[i].path = set->items[i].verified_path;
    }
    context->sources = set->bindings;
    context->source_count = set->count;
    return 1;
}
    ...
    for (source_index = 0; source_index < set->count; ++source_index) {
        ...
        for (url_index = 0; url_index < source->url_count; ++url_index) {
            ...
            if (snprintf(temporary_path, sizeof(temporary_path),
                         "%s/.cbs-fetch-XXXXXX",
                         cache_directory) >= (int)sizeof(temporary_path))
                return 0;

                       ...
                       const char *path)
{
    ...
    if (!read_blob(path, &part, &part_size) ||
        part_size > SIZE_MAX - *size) {
        free(part);
        return 0;
    }
    ...
    if (needed > *capacity) {
        ...
        while (next < needed) {
            if (next > SIZE_MAX / 2) { free(part); return 0; }
            next *= 2;
        }
        *data = realloc(*data, next);
        if (*data == NULL) { free(part); return 0; }
        *capacity = next;
    }
        ...
        if (sscanf(line, "f %31s %*u %*u %llu %64s %4095[^\n]", mode, &size,
            ...
            snprintf(path, sizeof(path), "%s/%s", root, relative) >=
                (int)sizeof(path) || !append_blob(payload, payload_size,
                                                   &capacity, path)) {
            fclose(file); free(*payload); *payload = NULL; return 0;
        }
    }
    if (ferror(file) || fclose(file) != 0) {
        free(*payload); *payload = NULL; return 0;
    }
    return 1;
}
                         ...
                         unsigned char **output, size_t *output_size)
{
    ...
    *output_size = ZSTD_compress(*output, bound, input, input_size, 19);
    if (ZSTD_isError(*output_size)) {
        free(*output); *output = NULL; return 0;
    }
    return 1;
}
                          ...
                          const char *package_path, const char *identity)
{
    ...
    size_t manifest_size, payload_size, manifest_compressed_size, payload_compressed_size;
    ...
    if (!manifest || !root || !package_path || !identity ||
        ...
        !cbs_digest_text((char *)manifest_data, manifest_size, manifest_digest) ||
    ...
    if (file != NULL && fwrite(header, 1, sizeof(header), file) == sizeof(header) &&
        ...
        fwrite(payload_compressed, 1, payload_compressed_size, file) ==
            payload_compressed_size && fclose(file) == 0)
        ok = 1;
    else if (file != NULL)
        fclose(file);
cleanup:
    free(manifest_data); free(payload); free(manifest_compressed);
                           ...
                           size_t identity_size)
{
    ...
    if (!read_blob(package_path, &data, &size) || size < 352 ||
        ...
        { free(data); return 0; }
    manifest_compressed_size = ZSTD_findFrameCompressedSize(data + 352,
                                                               size - 352);
    if (ZSTD_isError(manifest_compressed_size) || manifest_compressed_size > size - 352)
        { free(data); return 0; }
    ...
    if (manifest == NULL || payload == NULL)
        { free(data); free(manifest); free(payload); return 0; }
    written = ZSTD_decompress(manifest, (size_t)manifest_size,
    ...
    if (identity != NULL && identity_size != 0) {
        ...
        memcpy(identity, data + 160, copy); identity[copy] = '\0';
    }
    ...
    fclose(file); free(data); free(manifest); free(payload); return 1;
}
...
static int remove_tree(const char *path)
{
    ...
    struct dirent *entry;
    char child[4096];
    if (directory == NULL) return unlink(path) == 0 || errno == ENOENT;
    while ((entry = readdir(directory)) != NULL) {
        if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0)
            continue;
        if (snprintf(child, sizeof(child), "%s/%s", path, entry->d_name) >=
                (int)sizeof(child) || !remove_tree(child)) {
...
int cbs_cixpkg_extract(const char *package_path, const char *destination)
{
    ...
    char temporary[4096] = {0}, line[8192], type, mode_text[32], digest[65], relative[4096];
    ...
    if (!manifest || !payload) goto cleanup;
    written = ZSTD_decompress(manifest, manifest_size, data + 352,
                              manifest_compressed);
    if (ZSTD_isError(written) || written != manifest_size ||
        !cbs_digest_text((char *)manifest, written, digest) ||
        memcmp(data + 32, digest, 64) != 0) goto cleanup;
    written = ZSTD_decompress(payload, payload_size,
                              data + 352 + manifest_compressed,
                              payload_compressed);
    if (ZSTD_isError(written) || written != payload_size ||
        !cbs_digest_text((char *)payload, written, digest) ||
        memcmp(data + 96, digest, 64) != 0) goto cleanup;
    if (snprintf(temporary, sizeof(temporary), "%s.cbs-tmp-XXXXXX", destination) >=
    ...
    if (rename(temporary, destination) != 0) goto cleanup;
    ok = 1;
cleanup:
    if (file) fclose(file);
    if (descriptor >= 0) close(descriptor);
    if (!ok && temporary[0] != '\0') remove_tree(temporary);
    free(data); free(manifest); free(payload);
    return ok;
}

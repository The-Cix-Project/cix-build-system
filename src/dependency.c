                               ...
                               CbsDependencySet *set)
{
    ...
    for (index = 0; index < package->child_count; ++index) {
        const CbsNode *requires = package->children[index];
        if (requires->kind != CBS_NODE_REQUIRES)
            continue;
        for (group_index = 0; group_index < requires->child_count; ++group_index) {
            const CbsNode *group = requires->children[group_index];
...
int cbs_dependency_set_contains(const CbsDependencySet *set,

                        ...
                        const CbsExecutionContext *context)
{
    ...
    if (token_kind == CBS_TOKEN_CBS_VALUE) {
        const char *resolved = context_value(context, value + 1,
                                             strlen(value + 1), number,
                                             sizeof(number));
    ...
    while (*cursor != '\0') {
        ...
        resolved = context_value(context, opening + 2,
                                 ...
                                 sizeof(number));
        if (resolved != NULL)
            append_text(&result, &length, &capacity, resolved, strlen(resolved));
        cursor = closing + 1;
    }
    if (result == NULL)
...
static char *join_path(const char *left, const char *right)
{
    ...
    char *path = cbs_allocate(left_length + (size_t)separator + right_length + 1);
...
static char *resolve_executable(const char *program, const char *working_directory,
...
static void runtime_error(const CbsNode *run, const CbsExecutionContext *context,
...
int cbs_execute_run(const CbsNode *run, const CbsExecutionContext *context)
{
    ...
    executable = resolve_executable(program, context->working_directory,
                                    &environment);
    if (executable == NULL) {
        snprintf(message, sizeof(message), "cannot resolve executable `%s`", program);
    ...
    child = fork();
    if (child < 0) {
        snprintf(message, sizeof(message), "cannot create process; errno=%d", errno);
        runtime_error(run, context, "CPDL-E4001", message);
        free(executable);
        free(program);
        string_list_destroy(&arguments);
    ...
    if (wait_result < 0) {
        snprintf(message, sizeof(message), "cannot wait for process; errno=%d", errno);
        runtime_error(run, context, "CPDL-E4001", message);
        return 0;
    }
    ...
    if (!WIFEXITED(status) || WEXITSTATUS(status) != expected_status) {
        snprintf(message, sizeof(message), "process exited with status %d; expected %ld",
